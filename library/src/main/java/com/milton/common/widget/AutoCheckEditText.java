    ...
    public void afterTextChanged(Editable s) {
        if (s != null && s.length() > 0) {
            ...
            if (type == TYPE_OF_MOBILE) {
                isMatch = StringUtils.isMobileNO(s.toString());
            } else if (type == TYPE_OF_TEL) {
                isMatch = StringUtils.isTel(s.toString());
            } else if (type == TYPE_OF_EMAIL) {
                isMatch = StringUtils.isEmail(s.toString());
            } else if (type == TYPE_OF_URL) {
                isMatch = StringUtils.isUrl(s.toString());
            } else if (type == TYPE_OF_CHZ) {
                isMatch = StringUtils.isChz(s.toString());
            } else if (type == TYPE_OF_USERNAME) {
                isMatch = StringUtils.isUsername(s.toString());
            } else if (type == TYPE_OF_USER_DEFINE) {
                isMatch = StringUtils.isMatch(userRegx, s.toString());
            } else {
                return;
            }
            if (isMatch) {
